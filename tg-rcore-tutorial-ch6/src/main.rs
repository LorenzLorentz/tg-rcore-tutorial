...
mod impls {
    ...
    fn range_has_mapping(process: &ProcStruct, range: &Range<VPN<Sv39>>) -> bool {
        process
            .address_space
            .areas
            .iter()
    ...
    fn range_is_fully_mapped(process: &ProcStruct, range: &Range<VPN<Sv39>>) -> bool {
    ...
    fn read_user_cstr(current: &ProcStruct, path: usize) -> Option<String> {
        let ptr = current
            .address_space
    ...
    impl IO for SyscallContext {
        ...
        fn open(&self, _caller: Caller, path: usize, flags: usize) -> isize {
            let current = PROCESSOR.get_mut().current().unwrap();
            if let Some(string) = read_user_cstr(current, path) {
                if let Some(fd) =
        ...
        fn linkat(
            ...
            let current = PROCESSOR.get_mut().current().unwrap();
            let Some(oldpath) = read_user_cstr(current, oldpath) else {
                return -1;
            };
            let Some(newpath) = read_user_cstr(current, newpath) else {
                return -1;
            };
        ...
        fn unlinkat(&self, _caller: Caller, _dirfd: i32, path: usize, _flags: u32) -> isize {
            let current = PROCESSOR.get_mut().current().unwrap();
            let Some(path) = read_user_cstr(current, path) else {
                return -1;
            };
        ...
        fn fstat(&self, _caller: Caller, fd: usize, st: usize) -> isize {
            let current = PROCESSOR.get_mut().current().unwrap();
            let Some(file) = current.fd_table.get(fd).and_then(|file| file.as_ref()) else {
                return -1;
            };
            let file = file.lock();
    ...
    impl Process for SyscallContext {
        ...
        fn spawn(&self, _caller: Caller, path: usize, count: usize) -> isize {
            ...
            let (parent_pid, child_proc) = {
                ...
                let Some(name) = core::str::from_utf8(unsafe {
                ...
                let Some(child_proc) = ProcStruct::from_elf(elf) else {
                    return -1;
                };
                (parent_pid, child_proc)
            };
            let pid = child_proc.pid;
    ...
    impl Memory for SyscallContext {
        ...
        fn mmap(
            ...
            let current = PROCESSOR.get_mut().current().unwrap();
            if range_has_mapping(current, &range) {
                return -1;
            }
        ...
        fn munmap(&self, _caller: Caller, addr: usize, len: usize) -> isize {
            ...
            let current = PROCESSOR.get_mut().current().unwrap();
            if !range_is_fully_mapped(current, &range) {
                return -1;
            }
