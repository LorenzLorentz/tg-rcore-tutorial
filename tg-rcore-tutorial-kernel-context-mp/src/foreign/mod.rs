...
pub trait ForeignPortal {
    ...
    /// # Safety
    ///
    /// 调用者必须确保 `key` 对应的插槽已分配且有效，并且当前调用对该插槽拥有独占访问权。
    unsafe fn transit_cache(&self, key: impl SlotKey) -> &mut PortalCache;
}
...
impl ForeignContext {
    ...
    pub unsafe fn execute(&mut self, portal: &impl ForeignPortal, key: impl SlotKey) -> usize {
