...
fn lab_scenario() -> &'static str {
    option_env!("T2L4_SCENARIO").unwrap_or("mixed")
}

fn trace_enabled() -> bool {
    matches!(
        option_env!("T2L4_TRACE"),
...
impl SchedTraceCollector {
    ...
    fn print_report(&self, scheduler: SchedulerKind) {
        if self.completed.is_empty() {
            println!(
                "[t2l4-summary] scheduler={} scenario={} tasks=0 avg_wait_us=0 avg_turnaround_us=0 throughput_milli_per_s=0 p95_latency_us=0 p99_latency_us=0 starvation=0",
                scheduler.as_str(),
                self.scenario
            );
            return;
        ...
        println!(
            "[t2l4-summary] scheduler={} scenario={} tasks={} avg_wait_us={} avg_turnaround_us={} throughput_milli_per_s={} p95_latency_us={} p99_latency_us={} starvation={}",
            scheduler.as_str(),
            self.scenario,
            task_count,
            avg_wait_us,
        ...
        if self.trace_enabled {
            for event in &self.events {
                println!(
                    "[t2l4-trace] ts_ns={} reason={} prev_tid={} next_tid={} qlen={} slice_ns={}",
                    event.timestamp_ns,
                    event.reason.as_str(),
            ...
            for task in &self.completed {
                println!(
                    "[t2l4-task] tid={} wait_us={} turnaround_us={} runtime_us={} dispatches={} slices={} starvation={}",
                    task.tid.get_usize(),
                    task.wait_ns / 1_000,
...
impl SchedulerKind {
    fn from_env() -> Self {
        match option_env!("T2L4_SCHED").unwrap_or("rr") {
            "fcfs" | "FCFS" => Self::Fcfs,
            "sjf" | "SJF" => Self::Sjf,
            "rr" | "RR" => Self::Rr,
            "mlfq" | "MLFQ" => Self::Mlfq,
