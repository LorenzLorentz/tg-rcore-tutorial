...
本目录从 `tg-rcore-tutorial-ch5` 复制而来，作为 `ch5-pingpong` 的独立开发目录。基线内核能力仍然是 `ch5`：进程管理、`fork/exec/waitpid`、Shell、基础标准输入输出、stride 调度扩展。
...
`ch5-pingpong` 的目标不是再做一遍 `ch5 exercise`，而是在当前 `ch5` 基线之上补出“能承载双进程用户态游戏”的那一层能力。
...
当前仓库已经有一个独立图形组件 crate：

- [tg-rcore-tutorial-gfx](/Users/lorenzlorentz/tg-rcore-tutorial/tg-rcore-tutorial-gfx)

它现在已经提供：

- `Display`：基于 `VirtIO-GPU` 的 framebuffer 初始化和 `present()`
- `Canvas`：最小软件绘图接口
- `fill_rect` / `fill_triangle` / `fill_convex_polygon`

对 `pingpong` 来说，最直接的用法不是把它直接塞给用户态，而是先把它当成**内核侧显示后端**：

1. 在内核启动阶段初始化 `Display`
2. 维护一块游戏帧缓冲
3. 每帧调用 `Canvas` 画背景、球拍、球、分数区域
4. 调用 `present()` 刷到屏幕

如果你希望继续坚持“用户态游戏逻辑 + 用户态绘图”，推荐两种路线二选一：

- 路线 A：内核提供少量绘图 syscall，例如 `clear/fill_rect/present`
- 路线 B：内核把 framebuffer 映射到用户态，由用户程序直接写像素，内核只负责 flush

对 `ch5-pingpong` 来说，路线 A 实现更快；路线 B 更像真正的图形子系统。

### 3. 当前图形组件需要哪些扩展

当前 `tg-rcore-tutorial-gfx` 已经足够画 `tangram`，但离 `pingpong` 的长期复用还差几项：

- 把 `Display` 的 HAL / DMA / 物理地址转换从 `ch1` 式的恒等映射假设里解耦出来，适配 `ch5` 的页表和堆分配环境。
- 增加稳定的 framebuffer 生命周期管理，而不是只面向“一次初始化后常驻绘制”。
- 增加更适合 2D 游戏的 API，例如 `stroke_rect`、`fill_circle`、位图文字、简单 sprite blit。
- 支持 dirty-rect 或部分刷新，避免每帧都全屏重绘。
- 明确“内核侧绘图”和“用户态可见 framebuffer”的边界，避免后面迁移到 `ch8` 时推倒重来。
...
- 原始章节目录仍然是 [tg-rcore-tutorial-ch5](/Users/lorenzlorentz/tg-rcore-tutorial/tg-rcore-tutorial-ch5)
- 当前目录是 task3 的游戏分支：
  [tg-rcore-tutorial-t3l5-pingpong](/Users/lorenzlorentz/tg-rcore-tutorial/tg-rcore-tutorial-t3l5-pingpong)

后续如果你要真正开始实现，建议优先在这个目录内改，不要回改原始教学章节。
