...
- 新的、基于已恢复 `t4l45-baseline` 实体的完整评测见：
  - `../tg-rcore-tutorial-t4l45-baseline/evaluation-report.md`
- 当前文件保留旧内容，仅作历史记录。
...
This file has been superseded by the recovered-baseline evaluation at:

- `../tg-rcore-tutorial-t4l45-baseline/evaluation-report.md`

The newer report is the one backed by an actually reconstructed `t4l45-baseline` artifact plus isolated `current-smp1` and `current-smp2` evaluation copies.
